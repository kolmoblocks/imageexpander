...
void extractHeader(char* diff, unsigned &highResWidth, unsigned &highResHeight) {
...
vector<unsigned char> getBits(char *buffer, int start, int len){
    vector<unsigned char> res;    
    res.reserve(len);
    for (int i = len; i < start + len; i++){
...
bool checkFileHeader(char* diff){
    vector<unsigned char> v;
    string res = "";
    v = getBits(diff,0,8);
    res += (char)binToInt(v);
...
void enhance(char *lowResFileName, char *diffFileName) {
    ...
    vector<unsigned char> deltas, diffPixelVec, lowResImage)
    ...
    unsigned error = lodepng::decode(lowResImage, lowResWidth, lowResHeight, lowResFileName, LCT_RGB, 8);
